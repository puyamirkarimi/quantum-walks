...
def quantum_walk(n, U, normalise=True):
...
def evolution_operator(H_driver, H_problem, T, M):
    ...
    for i in range(1, M):
        t = i * (T/M)
        H = (1 - t/T)*H_driver + (t/M)*H_problem
        U = np.matmul(U, expm((-1.0j/M) * H))
    print("found U")
    return U
...
if __name__ == '__main__':
    ...
    U = evolution_operator(H_driver, H_problem, t_finish, M)

    probs = quantum_walk(n, U)
    ...
    plt.xlim(0, n)
    plt.ylabel("Normalised state probability, P(d)")
    plt.show()
