...
if __name__ == '__main__':
    ...
    for loop, i in enumerate(range(n_shifted*10000, (n_shifted+1)*10000)):                               # 10000 instances per value of n
        ...
        string_start_index = output.find('time ') + 5
        string_end_index = output.find('s\\n+1')
        timer = output[string_start_index: string_end_index]
        print(output)
        #states_count[loop] = states_visited

    # with open("adam_noGT_nondg_runtimes_"+str(n)+".txt", "ab") as f:         # saves runtimes using time.time()
    #     f.write(b"\n")
    #     np.savetxt(f, runtimes)
    #
    # with open("adam_noGT_nondg_counts_"+str(n)+".txt", "ab") as f:         # saves counts
    #     f.write(b"\n")
